...
struct BenevolesView: View {
    ...
    var body: some View {
        VStack {
            if (benevolesViewModel.benevoles.count < maximum && !benevolesViewModel.benevoles.contains(where: { $0.id == UserSession.shared.user!.id })) {
                Button("Participer") {
                    Task {
            ...
            if (benevolesViewModel.benevoles.contains(where: { $0.id == UserSession.shared.user!.id })) {
                Button("Quitter") {
                    Task {
